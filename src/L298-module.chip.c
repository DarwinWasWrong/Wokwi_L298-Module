...
// timer for pwm heartbeat
static void chip_timer_event_pwm_heartbeat(void *user_data);
...
void chip_init(void) {
 ...
 //
  ...
  // use PWM is the reverse of start state
...
const timer_config_t timer_config_motorB = {
  ...
  //   chip->use_PWM_ENA= !chip->start_state_ENA;
  //   chip->use_PWM_ENB= !chip->start_state_ENB;
  if (chip->use_PWM_ENA || chip->use_PWM_ENB)
  {
    // if the link isnt on both channels then this is required.
// pwm heartbeat timer - used to detect 255 or 0 PWM
// eg no pulse either HIGH or LOW
    const timer_config_t timer_config_pwm_heartbeat = {
    .callback = chip_timer_event_pwm_heartbeat,
    .user_data = chip,
  };
  // start the heartbeat timer
  timer_t timer_pwm_heartbeat = timer_init(&timer_config_pwm_heartbeat);
  timer_start(timer_pwm_heartbeat, 10000, true);
  }
...
for (int x=x_start;x < square_size + x_start; x++)
   {
    ...
    {
        uint8_t pixel_data = all_cogs[phase][pixel_spot_data] ;
       /* rgba_t  color =
            { 
        ...
        buffer_write(chip->framebuffer, (offset * x) + y * 4, &pixel_data, 4); 
        pixel_spot_data++;
    }
   }
}  
...
for (int x=x_start;x < square_size + x_start; x++)
   {
    ...
    // rgba_t  color1 =  rgba_t(test_pattern[pixel_spot_data] );
      uint8_t pixel_data =  board[pixel_spot_data] ; 
...
// heartbeat
void chip_timer_event_pwm_heartbeat(void *user_data) {
  chip_state_t *chip = (chip_state_t*)user_data;
  // heartbeat reset
